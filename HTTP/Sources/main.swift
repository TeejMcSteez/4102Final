...
/*
Loads the index.html from the bundle to send to the client
*/
func loadIndexHTML() -> String? {
    // Bundle.module is automatically synthesized by SwiftPM for targets with resources
    if let url = Bundle.module.url(forResource: "index", withExtension: "html") {
        do {
            return try String(contentsOf: url, encoding: .utf8)
        } catch {
            print("Error reading index.html: \(error)")
            return nil
        }
    } else {
        print("index.html not found in Bundle.module")
        return nil
    }
}
...
func run(port: UInt16 = 8080) throws {
    ...
    while true {
        ...
        let body = loadIndexHTML() ?? "{\"error\": \"Blank Body\"}"
